...
from region_file_updater_multi.components import misc as misc_comp
...
from region_file_updater_multi.utils.logging import NoColorFormatter, attach_file_handler, get_datetime_format
...
class RegionFileUpdaterMulti:
    def __init__(self):
        ...
        self.file_utilities = FileUtils(os.path.join(self.get_data_folder(), RECYCLE_BIN_FOLDER), self)
        ...
        self.group_manager = GroupManager(os.path.join(self.get_data_folder(), GROUP_FILE), self)

        misc_comp.set_rfum(self)
            ...
            for line in original.splitlines():
                ...
                if result is not None:
                    ...
                    processed.append(RText(line).c(RAction.suggest_command, command).h(
                        self.rtr(f'help_message.suggest', command)))

                    self.verbose(f'Rich help line: "{line}"')
                    self.verbose('Suggest prefix: {}'.format(f'"{suggest_prefix}"' if isinstance(suggest_prefix, str) else suggest_prefix))
    ...
    def ntr(
        ...
        try:
            ...
            if _mcdr_tr_allow_failure:
                ...
                raise  # KeyError(f'Error translate text "{translation_key}" to language {languages}, view the exception caught above to get more information')

    def register_event_listeners(self):
        self.server.register_event_listener(MCDRPluginEvents.PLUGIN_UNLOADED, lambda *args, **kwargs: self.unset_log())
