...
class ConfigurationBase(BlossomSerializable):
    @staticmethod
    def get_template(rfum: "RegionFileUpdaterMulti", path: PathLike) -> yaml.CommentedMap:
        rt_yaml = yaml.YAML(typ='rt')
        rt_yaml.width = 1048576
        try:
            with rfum.server.open_bundled_file(path) as f:
    ...
    def load(
        ...
        def log(translation_key, *args, _lb_rtr_prefix=TRANSLATION_KEY_PREFIX + 'config.', **kwargs):
            text = rfum.ktr(translation_key, *args, _lb_rtr_prefix=_lb_rtr_prefix, **kwargs)
    ...
    def save(
        ...
        def _save(safe_dump: bool = False):
            ...
            if safe_dump:
                with file_utils.safe_write(file_path, encoding=encoding) as f:
                    safe_yaml.dump(config_content, f)
                rfum.logger.warning("Validation during config file saving failed, saved without original format")
            else:
                formatted_config: yaml.CommentedMap
                if os.path.isfile(file_path):
                    formatted_config = rt_yaml.load(file_utils.lf_read(file_path, encoding=encoding))
                else:
                    formatted_config = self.get_template(rfum, bundled_template_path)
                for key, value in config_content.items():
                    formatted_config[key] = value
                with file_utils.safe_write(config_temp_path, encoding=encoding) as f:
                    rt_yaml.dump(formatted_config, f)
                try:
                    self.deserialize(safe_yaml.load(file_utils.lf_read(config_temp_path, encoding=encoding)))
                except (TypeError, ValueError):
                    log("Attempting saving config with original file format due to validation failure while " +
