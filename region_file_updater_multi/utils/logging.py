...
from region_file_updater_multi.mcdr_globals import PathLike, TRANSLATION_KEY_PREFIX, PB_LOGGER_NAME
...
def attach_file_handler(logger: logging.Logger, file_path: PathLike, formatter: logging.Formatter, encoding: str = 'UTF-8'):
...
def get_pb_logger(server: ServerInterface, file_path: str):
    logger = logging.Logger(PB_LOGGER_NAME)
    console_handler = SyncStdoutStreamHandler()
    console_handler.setFormatter(logging.Formatter("[%(name)s] [%(asctime)s] %(message)s", datefmt=get_time_format(server)))
    logger.addHandler(console_handler)
    attach_file_handler(logger, file_path, logging.Formatter('[%(asctime)s] %(message)s', datefmt=get_datetime_format(server)))
    return logger
